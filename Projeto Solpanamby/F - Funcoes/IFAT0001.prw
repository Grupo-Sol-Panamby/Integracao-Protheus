...
If Len( aIntegra ) > 0
	For nX := 1 To Len( aIntegra )
		...
		aAdd(aRetorno, {"PROCES"   , aIntegra[nX][5]})  //DESCRIวรO
		aAdd(aRetorno, {"ID_TRANS" , aIntegra[nX][6]})  //1 – INCLUIR | 2 – ALTERAR | 3 – EXCLUIR
		aAdd(aRetorno, {"TRANSAC"  , aIntegra[nX][7]})  //DESCRIวรO
		aAdd(aRetorno, {"ID_ORI"   , aIntegra[nX][8]})  //1 - PROTHEUS | 2 - PULSAR | 3 - SCTV | 4 - MอDIA+
		aAdd(aRetorno, {"ID_DES"   , aIntegra[nX][9]})  //1 - PROTHEUS | 2 - PULSAR | 3 - SCTV | 4 - MอDIA+
		...
		aAdd(aErro, {"PROCES"   , aIntegra[nX][5]})  //DESCRIวรO
		aAdd(aErro, {"ID_TRANS" , aIntegra[nX][6]})  //1 – INCLUIR | 2 – ALTERAR | 3 – EXCLUIR
		aAdd(aErro, {"TRANSAC"  , aIntegra[nX][7]})  //DESCRIวรO
		aAdd(aErro, {"ID_ORI"   , aIntegra[nX][8]})  //1 - PROTHEUS | 2 - PULSAR | 3 - SCTV | 4 - MอDIA+
		aAdd(aErro, {"ID_DES"   , aIntegra[nX][9]})  //1 - PROTHEUS | 2 - PULSAR | 3 - SCTV | 4 - MอDIA+
...
For nX := 1 To Len( aContFat )
	...
	//Verifica se precisa gerar pedido ou s๓ faturar o pedido jแ gerado
		...
		If SC5->( DbSeek( xFilial("SC5") + cPedido ) )
			For nFat := 1 To Len( aNotas )
				...
				For nItFat := 1 To Len( aItemFat )
					//ap๓s a gera็ใo do pedido o indice ้ alterado
